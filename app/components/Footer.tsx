      ...
      <div className="max-w-[1200px] mx-auto flex flex-col md:flex-row justify-between items-center gap-12">
        ...
        <div className="flex flex-col md:flex-row items-center gap-8 md:gap-16">
            <div className="flex flex-col items-center md:items-start gap-1">
                <p className="text-accent-gold text-xs uppercase tracking-widest font-bold mb-1">Contact</p>
                <a href={`mailto:${contact?.email}`} className="text-white/70 hover:text-white text-sm transition-colors">{contact?.email}</a>
                <a href={`tel:${contact?.phone}`} className="text-white/70 hover:text-white text-sm transition-colors">{contact?.phone}</a>
            </div>
            ...
            <div className="flex items-center gap-6">
                ...
                <a href={socials?.substack || "#"} target="_blank" rel="noopener noreferrer" className="text-white hover:text-accent-gold transition-colors"><SubstackIcon size={20} /></a>
            </div>
        </div>
      </div>
    </footer>
